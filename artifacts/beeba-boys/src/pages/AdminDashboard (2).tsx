...
function BookingsTab() {
  ...
  const statusStyle = (status: string) => {
    ...
    if (s === "pending")   return "text-yellow-500 border-yellow-500/50";
    if (s === "confirmed") return "text-green-500 border-green-500/50";
    if (s === "completed") return "text-blue-400 border-blue-400/50";
    return "text-muted-foreground border-border";
  };

  if (isLoading) return <div className="text-muted-foreground font-mono text-sm p-4">Loading bookings...</div>;
    ...
    <div>
      ...
      <div className="md:hidden space-y-4">
        {bookings.map((booking) => (
          <div key={booking.id} className="bg-card border border-border p-5 space-y-4">
            {/* Header row */}
            <div className="flex justify-between items-start gap-3">
              <div>
                <div className="font-medium text-sm">{booking.client_name}</div>
                <div className="text-xs text-muted-foreground font-mono mt-0.5">{booking.client_phone}</div>
              </div>
              <span className={`shrink-0 px-2 py-1 text-xs font-mono uppercase border ${statusStyle(booking.status)}`}>
                {booking.status}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="text-xs font-mono text-muted-foreground uppercase mb-1">Service</div>
                <div className="text-sm">{booking.service?.name ?? "—"}</div>
              </div>
              <div>
                <div className="text-xs font-mono text-muted-foreground uppercase mb-1">Stylist</div>
                <div className="text-sm">{booking.stylist?.name ?? "—"}</div>
              </div>
              <div>
                <div className="text-xs font-mono text-muted-foreground uppercase mb-1">Date</div>
                <div className="font-mono text-xs">{booking.appointment_date ? formatDate(booking.appointment_date) : "—"}</div>
              </div>
              <div>
                <div className="text-xs font-mono text-muted-foreground uppercase mb-1">Time</div>
                <div className="font-mono text-xs">{booking.appointment_time ? formatTime(booking.appointment_time) : "—"}</div>
              </div>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-background border border-border text-foreground text-sm px-3 py-2 outline-none focus:border-primary"
              >
        ...
        {bookings.length === 0 && (
          <div className="text-center py-12 text-muted-foreground font-mono text-sm">No bookings yet</div>
        )}
      </div>

      {/* ── DESKTOP: table layout ── */}
      <div className="hidden md:block w-full overflow-x-auto">
        <p className="text-xs text-muted-foreground mb-2">← Scroll to see all columns</p>
            ...
            {bookings.map((booking, idx) => (
              ...
              >
                ...
                <td className="px-3 py-3 w-[13%]">
                  <span className={`px-2 py-1 text-xs font-mono uppercase border ${statusStyle(booking.status)}`}>
                    {booking.status}
                  </span>
                </td>
              ...
              <tr>
                <td colSpan={6}>
                  <div className="flex flex-col items-center justify-center py-16 gap-4">
                    ...
                    <p style={{ fontFamily: "'DM Sans', sans-serif", color: "#6B6560" }}>No bookings yet</p>
                  </div>
                </td>
              </tr>
            )}
...
function TeamTab() {
  ...
  const handleSaveEdit = async () => {
    const { error } = await supabase
      ...
      .update({
        ...
        years_experience: editingMember.years_experience,
  ...
  return (
    <div>
      ...
      {editingMember && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border w-full max-w-lg p-6 md:p-8 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6 md:mb-8">
              <h2 className="text-xl md:text-2xl font-serif">Edit Member</h2>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-2 block">Speciality</label>
                <input
                  className="w-full bg-background border border-border px-4 py-3 text-foreground focus:border-primary outline-none"
                  value={editingMember.speciality ?? ""}
                  onChange={e => setEditingMember({ ...editingMember, speciality: e.target.value })}
                  placeholder="e.g. Fades & Beard Design"
                />
              </div>
              <div>
                <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-2 block">Years Experience</label>
                <input
                  type="number"
                  className="w-full bg-background border border-border px-4 py-3 text-foreground focus:border-primary outline-none"
                  value={editingMember.years_experience ?? ""}
                  onChange={e => setEditingMember({ ...editingMember, years_experience: Number(e.target.value) })}
                />
              </div>
              <div>
                <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-2 block">Bio</label>
                <textarea
                  className="w-full bg-background border border-border px-4 py-3 text-foreground focus:border-primary outline-none resize-none h-20"
              ...
              <div>
                <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-2 block">Photo URL</label>
                <input
                  type="url"
                  className="w-full bg-background border border-border px-4 py-3 text-foreground focus:border-primary outline-none"
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="accent-primary"
                />
                <label htmlFor="member-active" className="text-sm font-mono uppercase tracking-widest">
                  Active (visible on public site)
                </label>
              </div>
            </div>
      ...
      <div className="mb-6 flex justify-end">
        <button
          ...
          className="w-full md:w-auto bg-primary text-primary-foreground px-6 py-3 font-mono text-sm uppercase tracking-widest hover:bg-primary/90 transition-colors"
        >
          + Add Member
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {team.map(m => (
          <div key={m.id} className="bg-card border border-border p-5 relative group flex items-center gap-4 overflow-hidden min-w-0">
            <div className="w-14 h-14 bg-muted rounded-full overflow-hidden flex items-center justify-center shrink-0 border border-border">
              {m.photo_url
            ...
            <div className="flex-1 min-w-0">
              ...
              {m.years_experience && <div className="text-muted-foreground text-xs mt-0.5">{m.years_experience} yrs exp</div>}
            ...
            <div className="flex gap-2 shrink-0 md:absolute md:top-4 md:right-4 md:opacity-0 md:group-hover:opacity-100 md:transition-opacity">
              <button
              ...
              <button
                ...
                className="text-destructive text-xs font-mono border border-destructive/50 px-2 py-1 hover:bg-destructive/10 transition-colors"
              >
                Del
              </button>
            </div>
        ...
        {team.length === 0 && (
          <div className="col-span-2 text-center py-8 text-muted-foreground">No team members yet.</div>
        )}
      </div>
    </div>
  );
